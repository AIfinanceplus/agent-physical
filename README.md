...
| --- | --- |
| 生成的拆解台 | 28 个 |
| 零件行总数 | 4,285（其中**几何零件 1,577**，其余为物料清单行项） |
| 3D 几何 | 逐项目 GLB，合计 63 MB |
| 交互组件 | 与参照实现同一套（点选高亮 / 隔离 / 关节标记 / 爆炸 / 三张表） |
| 六项自洽断言 | **28/28 全部通过**（`pipeline/verify_teardowns.py`） |
| 画布渲染实测 | **29/29 有几何**（参照实现 + 28 个生成件，逐个切项目截图后数非背景像素） |
...
数据结构正确不等于**画得出来**。所以另做一次覆盖式实测：逐个切到每个项目、等 GLB 加载、
对 3D 画布区域截图，再用 PIL 数非背景像素占比。29 个全部非空（最低的 openAMR 4.71%、
OpenBionics 5.91%），没有一个是"数据齐、画面空"的。
`data/teardown-audit.json` 存六项断言结果，像素实测的原始截图与逐项目占比在
`~/agent-physical/.pixcheck/`（未入库）。
